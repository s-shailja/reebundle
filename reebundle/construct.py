...
from .events import Event, find_connect_disconnect_events
from dipy.segment.clustering import QuickBundles  # type: ignore
from dipy.segment.featurespeed import ResampleFeature  # type: ignore
from dipy.segment.metric import AveragePointwiseEuclideanMetric  # type: ignore
...
def constructRobustReeb(streamlines, eps, alpha, delta):
    ...
    for stream_i in range(len(streamlines)):
        stream_list.append(
            [0, True]
        )  # True signals if the index is going to be increased
        a_c = []
    ...
    for i in range(len(streamlines)):
        for j in range(i + 1, len(streamlines)):
            dic_t1, dic_t2 = find_connect_disconnect_events(
                i, j, streamlines[i], streamlines[j], eps
            )
        ...
        if stream_i not in del_s_id:
            ...
            if len(unique_cluster) == 1:
                ...
                if not cluster_edge.get(unique_cluster[0]):
                    ...
                    R[node_id][node_id + 1]["weight"] = count_trajectories[
                        unique_cluster[0]
        ...
        if len(assign_cluster[stream_i]) != 0 and stream_i not in del_s_id:
            ...
            if cluster_edge[x][1] in node_loc.keys():
                node_loc[cluster_edge[x][1]].append(
                    streamlines[stream_i][len(streamlines[stream_i]) - 1]
                )
            else:
    ...
    for node_key in node_loc.keys():
        ...
        for nk in node_loc[node_key]:
            ...
            n_y += nk[1]
            n_z += nk[2]
        node_loc_final[node_key] = [
            n_x / len(node_loc[node_key]),
            n_y / len(node_loc[node_key]),
            n_z / len(node_loc[node_key]),
        ]
    ...
    for stream_i in range(len(streamlines)):
        if stream_i not in del_s_id:
            ...
            for uc in range(len(unique_cluster) - 1):
                ...
                dist1 = distance(
                    node_loc_final[cluster_edge[unique_cluster[uc]][1]],
                ...
                if dist1 < dist2:
                    G_nodes.add_edge(
                        ...
                        cluster_edge[unique_cluster[uc + 1]][0],
                    )
                else:
                    G_nodes.add_edge(
                        cluster_edge[unique_cluster[uc]][1],
                        cluster_edge[unique_cluster[uc + 1]][1],
                    )
            ...
            for c in cluster:
                ...
                if node_id in node_loc_final:
                    node_loc_final[node_id] = [
                        node_loc_final[node_id][0] / 2 + node_loc_final[c][0] / 2,
                        node_loc_final[node_id][1] / 2 + node_loc_final[c][1] / 2,
                        node_loc_final[node_id][2] / 2 + node_loc_final[c][2] / 2,
                    ]
                else:
