"""
This file contains the function to find connect and disconnect events
    ...
    while ti < (len(t1)):
        ...
        while tj < (len(t2)):
            if (
                not flag_t1[ti]
                and not flag_t2[tj]
                and check_epsilon_distance(t1[ti], t2[tj], eps)
            ):
                # print(ti,"Start",tj)
                flag_t1[ti] = True
                flag_t2[tj] = True
                ...
                while flag_insert and last_j < len(t2) and first_j >= 0:
                    ...
                    if (first_i - 1 >= 0) and not flag_t1[first_i - 1]:
                        if (
                            first_j - 1 >= 0
                            and not flag_t2[first_j - 1]
                            and check_epsilon_distance(
                                t1[first_i - 1], t2[first_j - 1], eps
                            )
                        ):
                            first_i = first_i - 1
                            first_j = first_j - 1
                            ...
                            last_j + 1 < len(t2)
                            and not flag_t2[last_j + 1]
                            and check_epsilon_distance(
                                t1[first_i - 1], t2[last_j + 1], eps
                            )
                        ):
                            first_i = first_i - 1
                            last_j = last_j + 1
                            ...
                            for each_j in range(first_j, last_j + 1):
                                if check_epsilon_distance(
                                    t1[first_i - 1], t2[each_j], eps
                                ):
                                    first_i = first_i - 1
                                    flag_t1[first_i] = True
                                    flag_t2[each_j] = True
                    ...
                    if (first_j - 1 >= 0) and not flag_t2[first_j - 1]:
                        if (
                            first_i - 1 >= 0
                            and not flag_t1[first_i - 1]
                            and check_epsilon_distance(
                                t2[first_j - 1], t1[first_i - 1], eps
                            )
                        ):
                            first_j = first_j - 1
                            first_i = first_i - 1
                            ...
                            last_i + 1 < len(t1)
                            and not flag_t1[last_i + 1]
                            and check_epsilon_distance(
                                t2[first_j - 1], t1[last_i + 1], eps
                            )
                        ):
                            first_j = first_j - 1
                            last_i = last_i + 1
                ...
                if last_i + 1 < len(t1):  # beacuse it gets disconnected later
                    if dic_t1.get(last_i + 1):
                        dic_t1[last_i + 1].append(
                            Event("disconnect", t2_id, last_j + 1)
                        )
                    else:
                        dic_t1[last_i + 1] = [Event("disconnect", t2_id, last_j + 1)]
                if last_j + 1 < len(t2):
                    if dic_t2.get(last_j + 1):
                        dic_t2[last_j + 1].append(
                            Event("disconnect", t1_id, last_i + 1)
                        )
                    else:
