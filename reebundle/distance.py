...
def max_cost_score(
    ...
    max_score = (
        max_node_ins_cost + max_node_del_cost + max_edge_ins_cost + max_edge_del_cost
    )
...
def compute_graph_distance(H1, H2, node_loc_all1, node_loc_all2, eps, alpha, delta):
    ...
    dist_func = partial(
        ...
        alpha=alpha,
        delta=delta,
        scoring_func=my_scoring_fn,
        ins_cost=2 * eps * (1 + sub_r / 30),
    )
...
def distance(
    ...
    if (
        ...
        scoring_func = lambda n, ew, ed, gc, gr, eps, alpha, delta: (
            n,
            ew,
            ed,
        )  # default just returns back the node and edge scores
    ...
    dist = lambda p, q: np.linalg.norm(
        p[1]["position"] - q[1]["position"]
    )  # compute the Euclidean distance b/w nodes

    equivalency_mapping = (
        dict()
    )  # maps nodes in Gref to nodes in Gcmp, representing equality b/w them
    counterpart_nodes = (
        set()
    )  # set of all nodes in Gref with counterparts in Gcmp (through substitution or equality)
    ...
    valid_cand = (
        lambda nde: nde[0] not in counterpart_nodes
    ...
    not_found = (
        gref.nodes - counterpart_nodes
    )  # nodes in Gref that had no counterpart (equivalency or substitution) in Gcmp
    ...
    for e in gref.edges(data=True):
        ...
        if n1 in equivalency_mapping and n2 in equivalency_mapping:
            pce = (
                equivalency_mapping[n1],
                equivalency_mapping[n2],
            )  # finds (potential) corresponding edge based on equivalency mapping
        ...
        if (
            transform and add_edge
        ):  # Edge Insertion, if we are transforming the copy of Gcmp
